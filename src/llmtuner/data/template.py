...
class Template:
    ...
    def _encode(
        ...
        for turn_idx, (query, resp) in enumerate(history):
            ...
            query_ids = self._convert_inputs_to_ids(tokenizer, context=self.prompt, query=query, resp=resp, idx=str(turn_idx+1))
    ...
    def _convert_inputs_to_ids(
        ...
        for elem in context:
            if isinstance(elem, str):
                ...
                elem = elem.replace("{{query}}", query.strip(), 1) if query is not None else elem
                elem = elem.replace("{{resp}}", resp.strip(), 1) if resp is not None else elem
...
class Llama2Template(Template):
    ...
    def _encode(
        ...
        for turn_idx, (query, resp) in enumerate(history):
            ...
            query_ids = self._convert_inputs_to_ids(tokenizer, context=self.prompt, query=query, resp=resp)
            resp_ids = self._convert_inputs_to_ids(tokenizer, context=[resp])
